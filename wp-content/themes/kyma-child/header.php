...
<head>
	...
	<?php wp_head(); ?>
	<style id='top-menu-transparent' type='text/css'>
		.light_header div#navigation_bar{background:transparent !important;border-bottom:transparent !important;}
	</style>
	<!--<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js"></script>-->
	<!--<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>-->
	<script src="<?php echo get_bloginfo("template_url"); ?>-child/js/mousestop.min.js" type='text/javascript'></script>
</head>
<?php $class = "";
...
<body <?php body_class("menu_button_mode preloader3 menu_button_mode " . $class); ?> data-spy="scroll" data-target=".nav-bar" data-offset="50">
...
<div id="main_wrapper">
	<header id="site_header" class="nav-down">
		<div class="topbar <?php echo esc_attr($kyma_theme_options['topbarcolor']); ?>">
			<!-- class ( topbar_colored  ) -->
			<div class="content clearfix">
				<?php
				if ($kyma_theme_options['contact_info_header']) { ?>
				<div class="top_details clearfix f_left"><?php
					if ($kyma_theme_options['contact_phone']) {
					?>
					<span>
						<i class="fa fa-phone"></i>
						<span class="title"><?php _e('Call Us :', 'kyma') ?></span>
						<a href="tel:<?php echo esc_attr($kyma_theme_options['contact_phone']); ?>"><?php echo esc_attr($kyma_theme_options['contact_phone']); ?></a>
					</span>
					<?php
					}
					...
					<span>
						...
						<span class="title"><?php _e('Email :', 'kyma') ?></span>
						<a href="mailto:<?php echo sanitize_email($kyma_theme_options['contact_email']); ?>"><?php echo sanitize_email($kyma_theme_options['contact_email']); ?></a>
					</span>
					<?php } ?>
				</div>
		...
		<!-- End topbar -->
		<div id="navigation_bar" style="<?php if (get_header_image()) : ?> background-image:url('<?php header_image();?>'); <?php endif; ?>">
			<div class="cont-95">
				<div id="logo" class="col-xs-9 col-sm-4" <?php if ($kyma_theme_options['logo_layout'] == "right") {echo 'style="float:right;"';} ?>>
					<?php
					if ( version_compare( $wp_version, '4.5.2', '>=' ) ) {
						if (!strrpos(get_custom_logo(), "src")===false) {
							the_custom_logo();
						} else { ?>
					<a href="<?php echo esc_url(home_url('/')); ?>" title="<?php echo esc_attr(get_bloginfo('name', 'display')); ?>" rel="home"><h3><?php echo esc_attr(get_bloginfo('name')); ?></h3></a>
						<?php }
						} else { ?>
					<a href="<?php echo esc_url(home_url('/')); ?>" title="<?php echo esc_attr(get_bloginfo('name', 'display')); ?>" rel="home">
							<?php if ($kyma_theme_options['upload_image_logo']!="") { ?>
						<img id="logoimg" src="<?php echo esc_url($kyma_theme_options['upload_image_logo']); ?>" alt="<?php echo esc_attr(get_bloginfo('name', 'display')); ?>"/>
							<?php } else { ?>
				...
				<nav id="main_nav" class="nav-bar col-xs-3 col-sm-8 pull-right">
					<div id="nav_menu">
						<span class="mobile_menu_trigger">
							<a href="#" class="nav_trigger"><span></span></a>
						</span>
						<?php
						is_front_page()? $menu = "home-menu" : $menu = "";
						wp_nav_menu(array(
							'menu'						=> $menu,
							'theme_location'	=> 'primary',
							'menu_class'			=> 'clearfix horizontal_menu pull-right',
							'menu_id'					=> 'navy',
